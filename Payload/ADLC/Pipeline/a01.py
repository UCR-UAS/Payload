...
if args.getImage:
	...
	images_ret = open("recived.dat","r+w")
	for l in images_ret:
		if l[:len(l)-1] == args.getImage:
			print "exists"
			quit()
	getImage(air, '/DCIM/100OLYMP', args.getImage, headers)
	#images_ret.write(args.getImage+"\n")
	images_ret.close()
	#store args.getImage and check if it is in list
	quit()
...
def getImgNow(fi,res):
	getPage(air, 'exec_takemisc.cgi?com=stopliveview', headers)
	for l in fi:
		if l[:len(l)-1] == res:
			print "exists"
			return
	getImage(air, '/DCIM/100OLYMP', res, headers)
	fi.write(res+"\n")
	#store args.getImage and check if it is in list
	return
